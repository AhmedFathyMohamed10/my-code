...
def disease_search(request):
    try:
        # Retrieve search term and page number from request
        search_term = request.GET.get('search', '')
        page = int(request.GET.get('page', 1))

        # Initialize Elasticsearch search object
        s = Search(using=es, index='diseases_index')

        # Apply search query if a search term is provided
        if search_term:
            s = s.query(
                "multi_match",
                query=search_term,
